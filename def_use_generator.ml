...
(** Interface for functions that estimate the cost of assignments *)
module type CostEstimator = sig
  val get_cost: int -> rhs -> int
end
...
module HashtblGenerator (G : Graph) (E: CostEstimator)
  : DefUseGenerator
  ...
  let rec defuse_expr (gen: t) (e: expr): node =
    ...
    match e with
    | BinOp (e1, e2, op) ->
      let n1 = defuse_expr gen e1 in
      let n2 = defuse_expr gen e2 in
      gen.g <- G.connect gen.g (n1, new_node) max_int;
      gen.g <- G.connect gen.g (n2, new_node) max_int;
      new_node
    | InlineIf (e1, e2, e3) ->
      let n1 = defuse_expr gen e1 in
      let n2 = defuse_expr gen e2 in
      let n3 = defuse_expr gen e3 in
      gen.g <- G.connect gen.g (n1, new_node) max_int;
      gen.g <- G.connect gen.g (n2, new_node) max_int;
      gen.g <- G.connect gen.g (n3, new_node) max_int;
      new_node
    | _ -> new_node

  let defuse_rhs (gen: t) (r: rhs): node =
    ...
    in let new_node = create_node gen ntype in
    match r with
    | Expr e -> defuse_expr gen e
    | ArrayRead (i, e) ->
      let expr_node = defuse_expr gen e in
      gen.g <- G.connect gen.g (expr_node, sink) max_int;
      gen.g <- G.connect gen.g (source, new_node) max_int;
      new_node
    | PtrRead e ->
      let expr_node = defuse_expr gen e in
      gen.g <- G.connect gen.g (expr_node, sink) max_int;
      gen.g <- G.connect gen.g (source, new_node) max_int;
      gen.g <- G.connect gen.g (expr_node, new_node) max_int;
      new_node
  ...
  let rec defuse_cmd (gen: t) (c: cmd) (depth: int): node cmd_node =
    ...
    | Seq (c1, c2) ->
      ...
      SeqN(c1', c2')
    | If (e, c1, c2) ->
      let enode = defuse_expr gen e in
      let c1' = defuse_cmd gen c1 depth in
      let c2' = defuse_cmd gen c2 depth in
      gen.g <- G.connect gen.g (enode, sink) max_int;
      IfN(e, c1', c2')
    | While (e, body) ->
      let enode = defuse_expr gen e in
    ...
    | VarAssign (name, r) ->
      ...
      let rnode = defuse_rhs gen r in
      let cost = E.get_cost depth r in
      gen.g <- G.connect gen.g (rnode, varnode) cost;
      VarAssignN (name, r, varnode, rnode)
    | PtrAssign (e1, e2) ->
      let n1 = defuse_expr gen e1 in
      let _ = defuse_expr gen e2 in
      gen.g <- G.connect gen.g (n1, sink) max_int;
      PtrAssignN (e1, e2)
    | ArrAssign (i, e1, e2) ->
      let n1 = defuse_expr gen e1 in
      let _ = defuse_expr gen e2 in
